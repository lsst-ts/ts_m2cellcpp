...
/* PowerSubsystemConfig values based on this.
      - Motor Power Subsystem Configuration Information (typedef 'PowerSubsystem (cluster of 11 elements)
          ...
          - U32 output on max delay (ms) (unsigned long [32-bit integer (0 to 4,294,967,295)])  =
              “relay close delay” (50ms) + “breaker on time” (500ms) + “interlock output on delay”(50ms)=
   600ms
   ...
   level = 24V Based on “output voltage nominal level” * x%   x=”output voltage warning threshold level(%)”=
   5%
   ...
   = 85ms
          - U32 output voltage settling time (ms) (unsigned long [32-bit integer (0 to 4,294,967,295)])  =
   20ms
...
public:
    ...
    public:
        ...
        Feed() = delete;
        Feed(int bit0Pos, int bit1Pos, int bit2Pos)
                : _bit0Pos(bit0Pos), _bit1Pos(bit1Pos), _bit2Pos(bit2Pos) {}
