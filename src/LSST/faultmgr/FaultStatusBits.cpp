...
void FaultStatusBits::setBitAt(int pos) { setBit64(_bitmap, pos, true); }

void FaultStatusBits::unsetBitAt(int pos) { setBit64(_bitmap, pos, false); }
...
uint64_t FaultStatusBits::getMaskTelemetryOnlyControl() {
    if (_maskTelemetryOnlyControl == nullptr) {
        ...
        if (_maskTelemetryOnlyControl == nullptr) {
            ...
            fsm->setBitAt(FaultStatusBits::AXIAL_ACTUATOR_ENCODER_RANGE_FAULT);  // “Axial actuator encoder
                                                                                 // range fault”
            fsm->setBitAt(FaultStatusBits::TANGENT_ACTUATOR_ENCODER_RANGE_FAULT);  // “tangent actuator
                                                                                   // encoder range fault”
...
uint64_t FaultStatusBits::getMaskFaults() {
    if (_maskFaults == nullptr) {
        ...
        if (_maskFaults == nullptr) {
            ...
            fsm->setBitAt(
                    FaultStatusBits::LOSS_OF_TMA_COMM_ON_ENABLE_FAULT);  // “loss of TMA comm on ENABLE fault”
            ...
            fsm->setBitAt(FaultStatusBits::PARAMETER_FILE_READ_FAULT);  // “configurable parameter file read
                                                                        // error fault”
...
string FaultStatusBits::getEnumString(int enumVal) {
    switch (enumVal) {
        case STALE_DATA_WARN:
            return "STALE_DATA_WARN " + to_string(enumVal);
        case STALE_DATA_FAULT:
            return "STALE_DATA_FAULT " + to_string(enumVal);
        case BROADCAST_ERR:
            return "BROADCAST_ERR " + to_string(enumVal);
        case ACTUATOR_FAULT:
            return "ACTUATOR_FAULT " + to_string(enumVal);
        case EXCESSIVE_FORCE:
            return "EXCESSIVE_FORCE " + to_string(enumVal);
        case ACTUATOR_LIMIT_OL:
            return "ACTUATOR_LIMIT_OL " + to_string(enumVal);
        case ACTUATOR_LIMIT_CL:
            return "ACTUATOR_LIMIT_CL " + to_string(enumVal);
        case INCLINOMETER_W_LUT:
            return "INCLINOMETER_W_LUT " + to_string(enumVal);
        case INCLINOMETER_WO_LUT:
            return "INCLINOMETER_WO_LUT " + to_string(enumVal);
        case MOTOR_VOLTAGE_FAULT:
            return "MOTOR_VOLTAGE_FAULT " + to_string(enumVal);
        case MOTOR_VOLTAGE_WARN:
            return "MOTOR_VOLTAGE_WARN " + to_string(enumVal);
        case COMM_VOLTAGE_FAULT:
            return "COMM_VOLTAGE_FAULT " + to_string(enumVal);
        case COMM_VOLTAGE_WARN:
            return "COMM_VOLTAGE_WARN " + to_string(enumVal);
        case MOTOR_OVER_CURRENT:
            return "MOTOR_OVER_CURRENT " + to_string(enumVal);
        case COMM_OVER_CURRENT:
            return "COMM_OVER_CURRENT " + to_string(enumVal);
        case POWER_RELAY_OPEN_FAULT:
            return "POWER_RELAY_OPEN_FAULT " + to_string(enumVal);
        case POWER_HEALTH_FAULT:
            return "POWER_HEALTH_FAULT " + to_string(enumVal);
        case COMM_MULTI_BREAKER_FAULT:
            return "COMM_MULTI_BREAKER_FAULT " + to_string(enumVal);
        case MOTOR_MULTI_BREAKER_FAULT:
            return "MOTOR_MULTI_BREAKER_FAULT " + to_string(enumVal);
        case SINGLE_BREAKER_TRIP:
            return "SINGLE_BREAKER_TRIP " + to_string(enumVal);
        case POWER_SUPPLY_LOAD_SHARE_ERR:
            return "POWER_SUPPLY_LOAD_SHARE_ERR " + to_string(enumVal);
        case DISPLACEMENT_SENSOR_RANGE_ERR:
            return "DISPLACEMENT_SENSOR_RANGE_ERR " + to_string(enumVal);
        case INCLINOMETER_RANGE_ERR:
            return "INCLINOMETER_RANGE_ERR " + to_string(enumVal);
        case MIRROR_TEMP_SENSOR_FAULT:
            return "MIRROR_TEMP_SENSOR_FAULT " + to_string(enumVal);
        case MIRROR_TEMP_SENSOR_WARN:
            return "MIRROR_TEMP_SENSOR_WARN " + to_string(enumVal);
        case CELL_TEMP_WARN:
            return "CELL_TEMP_WARN " + to_string(enumVal);
        case AXIAL_ACTUATOR_ENCODER_RANGE_FAULT:
            return "AXIAL_ACTUATOR_ENCODER_RANGE_FAULT " + to_string(enumVal);
        case TANGENT_ACTUATOR_ENCODER_RANGE_FAULT:
            return "TANGENT_ACTUATOR_ENCODER_RANGE_FAULT " + to_string(enumVal);
        case MOTOR_RELAY:
            return "MOTOR_RELAY " + to_string(enumVal);
        case COMM_RELAY:
            return "COMM_RELAY " + to_string(enumVal);
        case HARDWARE_FAULT:
            return "HARDWARE_FAULT " + to_string(enumVal);
        case INTERLOCK_FAULT:
            return "INTERLOCK_FAULT " + to_string(enumVal);
        case TANGENT_LOAD_CELL_FAULT:
            return "TANGENT_LOAD_CELL_FAULT " + to_string(enumVal);
        case ELEVATION_ANGLE_DIFF_FAULT:
            return "ELEVATION_ANGLE_DIFF_FAULT " + to_string(enumVal);
        case MONITOR_ILC_READ_WARN:
            return "MONITOR_ILC_READ_WARN " + to_string(enumVal);
        case POWER_SYSTEM_TIMEOUT:
            return "POWER_SYSTEM_TIMEOUT " + to_string(enumVal);
        case PARAMETER_FILE_READ_FAULT:
            return "PARAMETER_FILE_READ_FAULT " + to_string(enumVal);
        case ILC_STATE_TRANSITION_FAULT:
            return "ILC_STATE_TRANSITION_FAULT " + to_string(enumVal);
        case CRIO_COMM_FAULT:
            return "CRIO_COMM_FAULT " + to_string(enumVal);
        case LOSS_OF_TMA_WARN:
            return "LOSS_OF_TMA_WARN " + to_string(enumVal);
        case LOSS_OF_TMA_COMM_ON_ENABLE_FAULT:
            return "LOSS_OF_TMA_COMM_ON_ENABLE_FAULT " + to_string(enumVal);
        case TEMP_DIFF_WARN:
            return "TEMP_DIFF_WARN " + to_string(enumVal);
        case CRIO_TIMING_FAULT:
            return "CRIO_TIMING_FAULT " + to_string(enumVal);
        case CRIO_TIMING_WARN:
            return "CRIO_TIMING_WARN " + to_string(enumVal);
        case USER_GENERATED_FAULT:
            return "USER_GENERATED_FAULT " + to_string(enumVal);
    }
