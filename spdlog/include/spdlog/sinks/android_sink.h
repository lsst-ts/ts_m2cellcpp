...
private:
    // There might be liblog versions used, that do not support __android_log_buf_write. So we only compile
    // and link against
    // __android_log_buf_write, if user explicitely provides a non-default log buffer. Otherwise, when using
    // the default log buffer, always log via __android_log_write.
