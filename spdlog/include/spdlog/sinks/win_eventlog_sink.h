...
// Writing to Windows Event Log requires the registry entries below to be present, with the following
// modifications:
...
//
// Since typically modifications of this kind require elevation, it's better to do it as a part of setup
// procedure. The snippet below uses mscoree.dll as the message file as it exists on most of the Windows
// systems anyway and happens to contain the needed resource.
//
...
public:
    ...
    static sid_t get_current_user_sid() {
        ...
        // Get the required size, this is expected to fail with ERROR_INSUFFICIENT_BUFFER and return the token
        // size
        DWORD tusize = 0;
...
protected:
    void sink_it_(const details::log_msg &msg) override {
        ...
        succeeded = ::ReportEventW(event_log_handle(), eventlog::get_event_type(msg),
                                   eventlog::get_event_category(msg), event_id_, current_user_sid_.as_sid(),
                                   1, 0, &lp_wstr, nullptr);
#else
