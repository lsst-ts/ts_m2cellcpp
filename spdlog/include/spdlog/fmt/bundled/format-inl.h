...
#pragma GCC diagnostic pop
    ...
    static constexpr uint64_t power_of_10_64[20] = {1, FMT_POWERS_OF_10(1ULL),
...
FMT_CONSTEXPR inline fp get_cached_power(int min_exponent, int& pow10_exponent) {
    ...
    int index = static_cast<int>(((min_exponent + fp::num_significand_bits - 1) * (significand >> shift) +
...
bool is_center_integer(typename float_info<T>::carrier_uint two_f, int exponent, int minus_k) FMT_NOEXCEPT {
    ...
    if (exponent > float_info<T>::divisibility_check_by_5_threshold) return false;
    if (exponent > float_info<T>::case_fc_upper_threshold) return divisible_by_power_of_5(two_f, minus_k);
