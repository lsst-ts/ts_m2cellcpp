    ...
    if constexpr (str[POS] == '{') {
        ...
        if constexpr (str[POS + 1] == '{') {
            ...
            if constexpr (arg_id_result.arg_id.kind == arg_id_kind::index) {
                ...
                return parse_replacement_field_then_tail<get_type<arg_index, Args>, Args, arg_id_end_pos,
                                                         arg_index, manual_indexing_id>(format_str);
...
format_to_n_result<OutputIt> format_to_n(OutputIt out, size_t n, const S& format_str, Args&&... args) {
    auto it =
            format_to(detail::truncating_iterator<OutputIt>(out, n), format_str, std::forward<Args>(args)...);
