    ...
    while (it != end) {
        ...
        if (it != end && *it == '.') {
            ...
            if ('0' <= c && c <= '9') {
                ...
                ++it;
                specs.precision =
                        static_cast<int>(visit_format_arg(detail::printf_precision_handler(), get_arg(-1)));
            } else {
                specs.precision = 0;
            }
        }
...
template <typename S, typename Char = char_t<S>>
inline auto vprintf(const S& fmt, basic_format_args<basic_printf_context_t<type_identity_t<Char>>> args)
        -> int {
