...
#define SPDLOG_LOGGER_CATCH(location)                                                                    \
        ...
        if (location.filename) {                                                                         \
            err_handler_(fmt_lib::format(SPDLOG_FMT_STRING("{} [{}({})]"), ex.what(), location.filename, \
                                         location.line));                                                \
        } else {                                                                                         \
...
protected:
    ...
    template <class T, typename std::enable_if<std::is_convertible<const T &, spdlog::wstring_view_t>::value,
                                               int>::type = 0>
